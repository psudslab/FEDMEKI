...
from transformers import LlamaForCausalLM, LlamaTokenizer, StoppingCriteria, StoppingCriteriaList, AutoTokenizer, AutoModelForCausalLM
...
VISION_TAGS = {
    "pos": {"image": "<image>", "time_series": "<ts>", "text":"", "demographics":"<demo>", "signal":"<signal>"},
    "sov": {"image": "<Img>", "time_series": "<Ts>", "text":"", "demographics":"<Demo>", "signal":"<Signal>"},
    "eov": {"image": "</Img>", "time_series": "</Ts>", "text":"", "demographics":"</Demo>", "signal":"</Signal>"},
}
...
class LAMMStoppingCriteria(StoppingCriteria):
    def __init__(self, stops, input_ids):
        ...
        # self.stops = [torch.tensor(stop).to('cuda:1') for stop in stops]  
...
#        device = "cuda:1"
        ...
        if self.encoder_pretrain.lower() == "clip":
            ...
            #clip_encoder = torch.load("/data/xiaochen/CSP_checkpoints/csp_clip.pt").to(device)
            clip_encoder = DeiTForImageClassificationWithTeacher.from_pretrained('facebook/deit-tiny-distilled-patch16-224').to(device)
            self.visual_preprocess = AutoFeatureExtractor.from_pretrained('facebook/deit-tiny-distilled-patch16-224')
            self.visual_encoder = clip_encoder
        ...
        self.llama_tokenizer = AutoTokenizer.from_pretrained(
            llm_ckpt_path, trust_remote_code=True
        )
    ...
    def initialize_language_model(self, llm_ckpt_path):
        ...
        if self.args.get('use_lightllm', False):
            ...
            print("loading medical ckpt")
            # self.llama_model = LlamaForCausalLM.from_pretrained(llm_ckpt_path)
            self.llama_model = AutoModelForCausalLM.from_pretrained(llm_ckpt_path, trust_remote_code=True)
    ...
    def clip_encode_image(self, inputs):
        ...
        if self.vision_feature_type == "global":
            with torch.no_grad():
                ...
                # embeddings = self.visual_encoder.visual.trunk.forward_features(inputs)
    ...
    def load_and_transform_image_data_clip(self, paths, device):
        ...
        for path in paths:
            if isinstance(path, Image.Image):
                ...
                image = Image.fromarray(image.pixel_array).convert("RGB")
                image_output = self.visual_preprocess(image, return_tensors="pt")['pixel_values'].squeeze().to(device)
            # elif image_path.startswith("http://"):
                # image = Image.open(requests.get(image_path, stream=True).raw)
            elif "RSNA" in path:
        ...
        return torch.stack(outputs, dim=0)  # B x 3 x 224 x 224
